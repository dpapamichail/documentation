...
- :guilabel:`Sales Price`: enter the recurring cost of the subscription that the customer pays per
   recurrence period.
...
A new customer subscription can be created by going to either the **Sales** or **Subscriptions** app
dashboards, then clicking :guilabel:`New`. On the quotation form, fill in the necessary fields such
as :guilabel:`Customer` and :guilabel:`Recurring Plan`, as well as the :guilabel:`Order Lines` tab.

Additionally, any of the following can also be added to the quotation:

- :guilabel:`Invoice Address` and :guilabel:`Delivery Address`, to differentiate between the address
  where a customer is invoiced and the address where a subscription that includes a physical good is
  sent. This option must be enabled by configuring the :doc:`Customer Addresses
  </applications/finance/accounting/customer_invoices/customer_addresses>` feature.
- :doc:`Quotation Template <sales/sales_quotations/quote_template>`, to speed up the quoting process
...
- :doc:`Pricelist <sales/products_prices/prices/pricing>`, to provide a unique discount, seasonal
  pricing, etc. (if one has been created.)
- :guilabel:`Payment Terms`, to specify a time frame by which the subscription must be paid once
