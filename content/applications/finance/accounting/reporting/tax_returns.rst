...
<accounting/tax-returns/advance-payments>` (based on the :doc:`fiscal localizations
...
- Action steps, such as :guilabel:`Review`, :guilabel:`Submit`, and :guilabel:`Pay`, and their
  status.
...
.. note::
   ...
   - When the :ref:`Review <accounting/tax-returns/vat-return-review>`, :ref:`Submit
     <accounting/tax-returns/vat-return-submit>`, or :ref:`Pay
     <accounting/tax-returns/vat-return-pay>` step is reached for a pending tax return, it appears
     in green in the action steps status summary.
...
- If all automatic tax validation checks have passed, the tax return has completed the
  :guilabel:`Review` step, and the :ref:`Submit <accounting/tax-returns/vat-return-submit>` step can
  proceed. If needed, click :guilabel:`See Checks` to view all checks.
- If any automatic tax validation checks are pending, the :guilabel:`Tax Checks` view displays the
  ...
  - :guilabel:`Bank Matching`: Not required for VAT report, but it helps identify any missing bills.
